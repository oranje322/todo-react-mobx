...
const AddNewButtonList:FC = observer(() => {



    const onClickVisionPopup = action(() => {
        sidebarStore.visiblePopup = !sidebarStore.visiblePopup
        sidebarStore.popupInputValue = ''
    })

    const onClickSelectColor = action((color: number) => {
        sidebarStore.selectedColor = color
    })

    const handleChangeInput = action((e: any) => {
        sidebarStore.popupInputValue = e.target.value
    })

    const onClickAddNewItem = action(() => {
        let newItem = {
            id: sidebarStore.folders.length,
            name: sidebarStore.popupInputValue,
            color: sidebarStore.selectColorMass[sidebarStore.selectedColor]
        }
        sidebarStore.addFolder(newItem)
        onClickVisionPopup()
    })
    ...
    return (
        <div className={'addListPopup'}>
            <div className="popupInput">
                <input value={sidebarStore.popupInputValue} onChange={handleChangeInput} className={'field'} type="text" placeholder={'Название папки'}/>
            </div>
            <div className="selectColor">
                {
                    sidebarStore.selectColorMass.map((color, index) => (
                        <div key={color+index}
                             onClick={ () => onClickSelectColor(index)}
                             className={cn(color, 'circle', {circle__active: sidebarStore.selectedColor === index} )}></div>))
                }
            </div>

            <button onClick={onClickAddNewItem} className={'sidebar__save-btn'}>
                Сохранить
            </button>

            <div onClick={onClickVisionPopup} className="krest">
