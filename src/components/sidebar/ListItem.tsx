...
const ListItem = observer(({id, name, color}: propsTypes) => {

    const ref = useRef(null)

    const onClickListItem = action(() => {
        sidebarStore.activeItem = id
    })

    const onClickDeleteItem = (e: any) => {
        e.stopPropagation()
        sidebarStore.deleteFolder(id)
    }


    return (

        <div onClick={onClickListItem} className={cn('listItem',{active: sidebarStore.activeItem === id}, {listHeader: id === 0})}>
            <div className={'listItemWrap'}>
                <div className={cn(color,{circle: id !== 0})}></div>
